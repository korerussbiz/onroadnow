...
async function getPrice(symbol) {
  ...
  const sources = [
    { name: 'CoinGecko', url: `https://api.coingecko.com/api/v3/simple/price?ids=${symbol.toLowerCase()}&vs_currencies=usd`, parse: d => d[symbol.toLowerCase()]?.usd },
    { name: 'Binance', url: `https://api.binance.com/api/v3/ticker/price?symbol=${symbol.toUpperCase()}USDT`, parse: d => parseFloat(d.price) },
    { name: 'Kraken', url: `https://api.kraken.com/0/public/Ticker?pair=${symbol.toUpperCase()}USD`, parse: d => { const k = Object.keys(d.result)[0]; return parseFloat(d.result[k].c[0]); } }
  ];
...
module.exports = async (req, res) => {
  ...
  if (url === '/api/health' && method === 'GET') {
    ...
    return res.status(200).json({ status: missing.length ? 'warning' : 'ok', checks, missing });
  }
  ...
  if (url === '/api/trade/execute' && method === 'POST') {
    ...
    if (!symbol || isNaN(amountUSD) || amountUSD <= 0) return res.status(400).json({ error: 'Invalid amount' });
    try {
      const price = await getPrice(symbol);
